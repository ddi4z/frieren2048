	...
	for (var i = 0; i < 4; i++) {
		for (var j = 0; j < 4; j++) {
			...
			if (nums[i][j] == 0) {
				...
				numberCell.css('background-image', getNumberBackgroundImage(nums[i][j])); // 设置背景图片
				numberCell.css('color', 'transparent'); // 隐藏文本颜色
			}
			...
			if(nums[i][j]!=0){
				for(var k=0;k<j;k++){
					if(nums[i][k]==0 && noBlockHorizontal(i,k,j,nums) ){//第I行的地k-j列是否有障碍物
						...
						nums[i][k]=nums[i][j];
						$('#number-cell-'+i+'-'+k).css('backgroundImage', getNumberBackgroundImage(nums[i][k]));
						nums[i][j]=0;
						break;
			...
			if(nums[i][j]!=0){
				for(var k=3;k>i;k--){
					if(nums[k][j]==0 && noBlockVertical(j,i,k,nums)){ //第j列的第i-k行之间是否有障碍物
						showMoveAnimation(i,j,k,j);
						nums[k][j]=nums[i][j];
						$('#number-cell-'+i+'-'+j).css('backgroundImage', getNumberBackgroundImage(nums[i][j]));
						nums[i][j]=0;
						break;
